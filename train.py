...
def train(args):
    ...
    # train_set[0]代表第0个数据，train_set[0][0]表示【embedding, localization, solubility, localization_solubility_metadata['metadata']】中的第0个
    if len(train_set[0][0].shape) == 2:  # if we have per residue embeddings they have an additional length dim，需要补全
    ...
    # 实例化模型，传入embeddings和config里面的model_parameters参数
